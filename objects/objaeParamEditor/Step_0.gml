...
if grabbed!=-1{ //dragging a point
    ...
    if !mouse_check_button(mb_left){
        ...
        var p = ds_list_find_value(points,grabbed),
            ...
            yy = remap_value(mouse_y+grab_y,gt,gb,gmax,-gmax);
...
/**/
    ...
    for(var j=0;j<cn;j+=1){
        ...
        for(var i=0;i<n;i+=1){
            ...
            var px = remap_value(ds_map_find_value(pp,"x"),0,100,gl,gr),
                py = remap_value(ds_map_find_value(pp,"y"),-gmax,gmax,gb,gt);
            if !is_string(ppx){
    ...
    var xx = remap_value(mouse_x,gl,gr,0,100),
        yy = remap_value(mouse_y,gt,gb,gmax,-gmax);
        for(var i=0;i<n;i+=1){
            ...
            var pxx = remap_value(px,0,100,gl,gr),
                pyy = remap_value(ds_map_find_value(pp,"y"),-gmax,gmax,gb,gt);
...
if blend!=-1{
    blend_highlight = -1;
    if curve_highlight==-1{ //no curve highlighted, let's highlight blends!
