...
if audio_debug_on{// and instance_exists(objLevel){
        ...
        for(var i=0;i<ds_list_size(playing);i+=1){
            ...
            if emit!=-1{
					...
					str+="#(emitter| x:"+string((ex))+", y:"+string((ey))+", z:"+string(ez)
						+", dist:"+string(sqrt(sqr(ex)+sqr(ey)+sqr(ez)))+", size "+string(aemitter_size)+"/"+string(aemitter_atten)+", pan "+string(100*aemitter_pan)+"%"+")"}
...
draw_set_color(c_white);
if beatEvent(){draw_set_color(c_red);}
